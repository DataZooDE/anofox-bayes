...
The family that ran is on the table too, as `__family__`: `1` for `hier_negbin`,
`2` for `censored_aft`, `3` for `pooled_gaussian`, `5` for `payer_alive`, `7` for
...
ran is on the table too, as `__family__`: `1` for `hier_negbin`, `3` for
`pooled_gaussian`, `5` for `payer_alive`, `7` for `conjugate_anomaly` — the catalog
F-numbers, decoded by
